...
<head>
	...
	<script type="text/javascript">
		$(function(){
			...
			/* 新建 */
			$(".common_button1").click(function(){
				var id=$("#123input").val();
				var todo =$("#todo").val();
				var date=$("#date").val();
				
				var url = "${ctp}/plan/new-ajax";
				var args = {"id":id,"todo":todo,"date":date};
				
				$.post(url,args,function(data){
					for(var i=0;i++;i<data.salesPlans){
...
<body class="main">
	...
	<div class="button_bar">
					...
					<tr id="plan-${plan.id}">
						...
						<td class="list_data_ltext">
							...
							<c:if test="${plan.result == null }">
								...
								<button class="common_button3" id="save-${plan.id}">
									保存
								</button>
								<input class="10jqinput" type="hidden" value="${chance.id}"/>
								
								
								<button class="common_button2" id="delete-${plan.id}">
									删除
								</button>
								<input class="jqkinput" type="hidden" value="${plan.id}"/>
		...
		<div class="button_bar">
			<button class="common_button1" name="id" value="${chance.id}">
				新建
			</button>
		</div>
		<input id="123input" type="hidden" name="id" value="${chance.id}" />
